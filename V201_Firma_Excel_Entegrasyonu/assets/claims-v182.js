...
function companyFields(){
 ...
 const reportInfo=kind==='firma'?window.CepheProFirma199.periodInfo({period:period(),model:m,project,readOnly:readOnly()}):null;
 const resolve=key=>reportInfo?reportInfo[key]??'':(readOnly()?[snapshot[key]]:[period().info191?.[key],snapshot[key],m[key],project[key]]).find(v=>v!=null&&String(v).trim()!=='')??'';
 return [...base,...extra].map(([key,label])=>({key,label,value:resolve(key)}));
}
...
function render(){
 ...
 $('cl-body').querySelectorAll('[data-cl-info]').forEach(input=>input.oninput=()=>{(per.info191||={})[input.dataset.clInfo]=input.value;if(kind!=='firma'||!window.CepheProFirma199.periodFieldKeys.has(input.dataset.clInfo))m[input.dataset.clInfo]=input.value;changed();});
...
async function excel(){
 ...
 if(!flushClaimInputs())return;
 exporting=true;
 const button=$('cl-excel'),label=button?.textContent||'Excel ve Ekleri İndir';
 if(button){button.disabled=true;button.textContent='Excel hazırlanıyor…';button.setAttribute('aria-busy','true');}
 const job={kind,number:period().number,month:model().month,id:period().id,owner:recordOwner,session:owner,version:changeVersion};
 const assertCurrent=()=>{
  if(!flushClaimInputs())throw Error('Geçersiz hakediş girişini düzeltin.');
  if(kind!==job.kind||owner!==job.session||recordOwner!==job.owner||period().id!==job.id||period().number!==job.number||model().month!==job.month||changeVersion!==job.version)throw Error('Hakediş seçimi veya veriler değişti. İndirmeyi yeniden başlatın.');
 };
 try{
  claimNotice(job.number+'. hakedişin Excel dosyası hazırlanıyor…');
  const {wb,extra=[]}=await workbook();assertCurrent();
  if(job.kind==='firma'&&(wb.worksheets.length!==5||wb.worksheets[0].name!=='Hakediş İcmali (Kapak)'))throw Error('Firma hakedişinin beş sayfalı raporu oluşturulamadı.');
  const bytes=await wb.xlsx.writeBuffer();assertCurrent();
  const prefix=job.kind==='firma'?'Firma':'Taseron',base=prefix+'_Hakedis_'+String(job.number).padStart(2,'0')+'_'+job.month,name=base+'.xlsx';
  if(extra.some(e=>e.file)){
   if(!window.JSZip)await new Promise((ok,no)=>{
    const script=document.createElement('script'),timer=setTimeout(()=>{script.remove();no(Error('Ek dosya modülü zamanında yüklenemedi.'));},20000);
    script.src='/assets/jszip.min.js';script.onload=()=>{clearTimeout(timer);ok();};script.onerror=()=>{clearTimeout(timer);script.remove();no(Error('Ek dosya modülü yüklenemedi.'));};document.head.append(script);
   });
   assertCurrent();
   const zip=new window.JSZip();zip.file(name,bytes);
   extra.forEach((e,i)=>{if(e.file){
    const match=String(e.file.data||'').match(/^data:[^,]*;base64,([A-Za-z0-9+/=\r\n]*)$/);
    if(!match)throw Error((e.file.name||'Ek dosya')+' okunamadı. Kaydı kontrol edin.');
    const filename=String(e.file.name||'ek').replace(/[\\/:*?"<>|\x00-\x1F]/g,'_').replace(/^\.+/,'_');
    zip.file('Ekler/'+(e.periodNumber||job.number)+'.Hakediş/'+(i+1)+'-'+filename,match[1],{base64:true});
   }});
   const archive=await zip.generateAsync({type:'blob'});assertCurrent();
   download(archive,base+'_ve_Ekler.zip');
  }else download(new Blob([bytes],{type:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'}),name);
  claimNotice(job.number+'. hakediş için Excel'+(extra.some(e=>e.file)?' ve ekleri':'')+' hazırlandı.');
 }catch(e){claimNotice('Excel oluşturulamadı: '+e.message);toast('Excel oluşturulamadı: '+e.message);}
 finally{exporting=false;const b=$('cl-excel');if(b){b.disabled=deleting;b.textContent=label;b.removeAttribute('aria-busy');}}
}
...
function capturePeriod(){
 ...
 per.snapshot190={savedAt:per.snapshot191.savedAt,rows:copy(rows),...t};per.dirty190=false;if(kind==='firma')captureFirmaReport199();return per.snapshot191;
}
...
function captureFirmaReport199(){
 ...
 s.reportVersion199=1;s.reportVersion200=1;s.reportVersion201=1;
...
function firmaWorkbook199(){
 const api199=window.CepheProFirma199;if(!api199?.periodPayload)throw Error('Firma rapor modülü yüklenemedi.');
 currentReport();captureFirmaReport199();
 const report=currentReport(),info=Object.fromEntries(companyFields().map(f=>[f.key,f.value]));
 const payload=api199.periodPayload({...report,info,contractRows:readOnly()?[]:firmaContractRows199(),readOnly:readOnly()});
 return api199.makeWorkbook(ExcelJS,payload);
}
